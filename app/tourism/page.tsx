...
import { MOCK_TOURISM, MOCK_LOCATIONS } from '@/lib/mock-data'

const TYPES = ['All', ...Array.from(new Set(MOCK_TOURISM.map(t => t.type)))]
...
export default async function TourismPage({
  ...
  const filtered = MOCK_TOURISM.filter(t => {
    const matchType = !type || type === 'All' ? true : t.type === type
    const matchLoc = !location ? true : t.location.toLowerCase().includes(location.toLowerCase())
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="flex flex-wrap gap-2">
            {TYPES.map(t => (
              <Link key={t} href={t === 'All' ? `/tourism${location ? `?location=${location}` : ''}` : `/tourism?type=${encodeURIComponent(t)}${location ? `&location=${location}` : ''}`}
          ...
          <div className="flex gap-2 flex-wrap sm:ml-auto">
            {MOCK_LOCATIONS.slice(0, 5).map(loc => (
              <Link key={loc.slug}
                href={`/tourism?location=${loc.slug}${type ? `&type=${type}` : ''}`}
        ...
        {filtered.length > 0 ? (
            ...
            {filtered.map(spot => (
              <div key={spot.id}
                ...
                <div className="h-52 overflow-hidden bg-gray-100 relative">
                  <img src={spot.image_url} alt={spot.name} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent" />
                  <span className={`absolute top-3 left-3 text-[10px] font-bold uppercase px-2.5 py-1 rounded-full text-white ${
                    spot.type === 'Nature' ? 'bg-green-500' :
                    spot.type === 'Beach' ? 'bg-blue-500' :
                    spot.type === 'Heritage' ? 'bg-amber-500' :
                    spot.type === 'Adventure' ? 'bg-red-500' : 'bg-purple-500'
                  }`}>{spot.type}</span>
                ...
                <div className="p-5">
                  <h3 className="font-bold text-base text-gray-900 group-hover:text-[#1428ae] transition-colors">{spot.name}</h3>
                  <p className="text-xs text-gray-500 mt-2 leading-relaxed line-clamp-3">{spot.description}</p>
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {spot.tags.map(tag => (
                      <span key={tag} className="text-[9px] font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">{tag}</span>
                    ))}
                  </div>
                  <Link href={`/${spot.location.toLowerCase().replace(/\s+/g, '-')}`}
                    className="block mt-4 text-center text-xs font-semibold text-[#1428ae] py-2 border border-[#1428ae]/20 rounded-lg hover:bg-[#1428ae] hover:text-white transition-colors">
                    See Properties in {spot.location} →
                  </Link>
                </div>
              </div>
            ))}
          ...
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="font-semibold text-gray-600">No destinations match your filters</p>
