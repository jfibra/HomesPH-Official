...
export async function GET(request: NextRequest) {
  ...
  const upstream = new URL(`${BASE_URL}/cities`)
  const countryId = searchParams.get('country_id')
  const provinceId = searchParams.get('province_id')
  if (countryId) upstream.searchParams.set('country_id', countryId)
  if (provinceId) upstream.searchParams.set('province_id', provinceId)
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch cities', detail: (err as Error).message },
      { status: 500 }
    )
  }
}
